...
const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      try {
        // Load sample data
        const { datasets: sampleDatasets, charts: sampleCharts } = loadSampleData();
      ...
      <div className="space-y-8" style={{ fontFamily: 'Poppins, sans-serif' }}>
              ...
              {recentCharts.length > 0 ? (
                <div className="space-y-4">
                  {recentCharts.slice(0, 2).map((chart) => {
                    ...
                    const chartDataset = recentDatasets.find(d => d.id === chart.datasetId);
                    const chartData = chartDataset?.rows || [];
                    
                    return (
                      <div key={chart.id} className="h-64">
                        <ChartComponentSimple chart={chart} data={chartData} />
                      </div>
                    );
                  })}
                </div>
        ...
        <GeminiChatPanel
          ...
          onSuggestChart={(config) => {
            ...
            // Could redirect to charts page or create chart directly
          }}
          onGenerateReport={(config) => {
            console.log('Report generation:', config);
            // Could redirect to reports page or generate report directly
          }}
        />
      </div>
    </DashboardLayout>
